        ...
        <Container>
            ...
            <Dialog open={open} onClose={handleClose}>
                ...
                <DialogContent id='ticket-dialog' className="footer hide-on-print">
                    ...
                    <Typography variant='h5'>{moment.utc(props.ticket.ticketType.eventRecord.event_starttime).local().format('DD.MM.YYYY HH:MM')} - {moment.utc(props.ticket.ticketType.eventRecord.event_endtime).local().format('DD.MM.YYYY HH:MM')}</Typography>
                    <Typography mb={4} variant='h5' >{props.ticket.ticketType.name}</Typography>
                    <Typography variant='caption'>Myyntiaika: {moment.utc(props.ticket.salesEvent.sale_date).local().format('DD.MM.YYYY HH:MM')} (id: {props.ticket.salesEvent.salesevent_id})</Typography>
                </DialogContent>
                <DialogActions>
